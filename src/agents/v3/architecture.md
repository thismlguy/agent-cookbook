# Airline agent v3 — orchestrator + specialist subagents

v3 is the first version that **splits the agent into multiple agents**. v1
and v2 were single ReAct loops with progressively better prompts; the
shape v3 targets is *production with small/fast models*, where two
properties matter that v1/v2 don't optimize for:

1. **Per-call prompts must stay short.** Small models lose accuracy as
   prompt length and rule density grow. v2's ~1500-word system prompt
...
The defining design decision in v3.
...
and the one most often mis-applied in v1/v2 evals, so it's the
...
- LangGraph runtime; `langgraph.prebuilt.create_react_agent` for the
  ...
  invoked by tools.)
- Per-session `Store` (in-memory, mutable). v3 adds `Store.pending_actions`
...
- New subdirectory: `src/agents/v3/subagents/` housing one Python file
...
- **v1 — baseline.** Single ReAct loop, flat-markdown policy, 10 tools.
- **v2 — better-prompted.** Same loop. XML-structured policy, operating
  principles, response style. Pure prompting changes.
- **v3 — orchestrator + specialists (this version).** Single ReAct loop
...
1. *v2 demonstrated the ceiling of pure prompting on a strong model.*
   Tone and routing move; rule application largely doesn't.
2. *v3 demonstrates the production shape when small/fast models are a
