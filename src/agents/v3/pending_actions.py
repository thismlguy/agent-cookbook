"""Pending-action store + execute helper for v3.

v3 splits every policy-loaded write into:
...
`execute()` re-reads at write time. See `src/agents/v3/architecture.md`.
"""
...
class PendingBook(_PendingBase):
    ...
    def execute(self, store: Store) -> dict[str, Any]:
        from src.agents.v1.tools import make_tools as _v1_make_tools
...
# Re-exported so other v3 modules can construct rows without a circular import path.
__all__ = [
    "FlightRef",
    "PendingPassenger",
    "PaymentRef",
