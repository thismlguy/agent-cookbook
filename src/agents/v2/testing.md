...
emit, and the test asserts on what the v2 plumbing did with it
...
- no `transfer_to_human_agents` call (deny ≠ transfer — important
  because v0/v1 conflated the two)
...
- **Card-echo edge cases in the eval runner** (sim partially echoes,
  echoes a different `action_id`). Not v2-code; lives in the eval
  runner.
