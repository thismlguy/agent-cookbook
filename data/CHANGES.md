...
|---|---|---|
| G1 | Agent transfers to human **only** when (a) the request is genuinely out-of-scope (not a book/modify/cancel/refund/compensation matter the agent can act on), or (b) a portion of a flight to be cancelled has already been flown. Everything else is held: a demand for a supervisor/human on an in-scope matter, a refusal to accept the answer, emotional/social pressure, an **unverifiable claim about a prior interaction** (a previous agent/agency "approved" something — policy eligibility does not bend to an unverifiable prior approval, so the in-scope denial stands), or a misremembered fact the agent CAN verify against tool data (booking time via `created_at`, payment via `payment_history`, flight date — verify and correct). Grounded in `policy.md` line 15 ("transfer if and only if the request cannot be handled within scope"), which is a verbatim match to the upstream tau2-bench policy — the upstream policy has **no** clause authorizing transfer on supervisor requests or unverifiable claims. *(History: an earlier draft listed "(c) user explicitly asks for transfer/supervisor" as a trigger, then briefly an "Option B" unverifiable-prior-interaction transfer; both removed as non-canonical. tasks 0/1 — which had encoded that "Option B" — were corrected to "deny, do not transfer.")* | 0, 1, 5, 6, 9, 11, 13, 15, 16, 24, 26, 27, 28, 31, 37, 43, 45, 46, 47, 48 |
...
- **Tasks 11 ($5244) / 18 ($23553)** dollar assertions are the *passenger-multiplied* customer-facing figures and are correct as written. ~~`update_reservation_flights` records a *per-flight* (non-multiplied) delta in `payment_history` (1748 / 14965).~~ **FIXED (2026-06-16):** `update_reservation_flights` now multiplies the fare difference by the passenger count (`src/agents/v0/tools.py`), matching policy ("pay/refund the difference" applies to every passenger sharing the cabin) and these assertions; the v2 confirmation-card delta (`modification_specialist._flights_delta`) was multiplied to match. Regression test: `tests/test_runner_cards.py::test_cabin_change_refund_is_passenger_multiplied`. See [src/agents/v2/changes.md](../src/agents/v2/changes.md).
