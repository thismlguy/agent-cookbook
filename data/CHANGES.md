...
|---|---|---|
| G1 | Agent transfers to human only when (a) the request is genuinely out-of-scope (not a book/modify/cancel/refund/compensation matter the agent can act on), (b) a portion of a flight to be cancelled has already been flown, or (c) the user invokes an **unverifiable claim about a prior interaction** (a previous agent/agency "approved" something) that cannot be checked against any tool — only a human has call-history access, so verifying it is out of scope ("Option B"; tasks 0, 1). What is **NOT** a transfer trigger: a bare demand for a supervisor/human on an in-scope matter, a refusal to accept the answer, emotional/social pressure, or a misremembered fact the agent CAN verify against tool data (booking time via `created_at`, payment via `payment_history`, flight date) — those are held/corrected, per `policy.md` line 15 ("transfer if and only if the request cannot be handled within scope"). *(Earlier draft listed "(c) user explicitly asks for transfer/supervisor" as a trigger; corrected — a bare supervisor request is not a trigger, contradicting tasks 5/45/47, but an unverifiable prior-interaction claim is, per tasks 0/1.)* | 0, 1, 5, 6, 9, 11, 13, 15, 16, 24, 26, 27, 28, 31, 37, 43, 45, 46, 47, 48 |
