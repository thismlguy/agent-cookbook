# v2 — Policy-aware agent architecture

Companion to `data/policy.md` and `data/decision-tree.md`. This document
proposes the structural shape of v2 — the version of the agent whose job
is to *reliably apply* the policy, not just hold it in a prompt.

v0 (`src/agents/v0/architecture.md`) and v1 (`src/agents/v1/architecture.md`)
both use a single LangGraph `create_react_agent` loop with the same 10 tools.
v0 ships the policy as flat markdown; v1 wraps it in XML and adds
...
The question this doc answers: **what structural change in v2 makes
...
These are settled by v0/v1 and not up for debate here:
...
- Universal protocol (one action per turn, confirm-before-write, tools
  ...
  the ReAct loop. v2 keeps this.

---

## Two cross-cutting layers (recap from `decision-tree.md`)

Whatever structure v2 adopts, these two layers apply to every turn:
...
Three architectural shapes are credible for v2. They differ in *where the
policy gets enforced* and *how many LLM calls happen per turn*.

### Option A — Single agent + validator tools (the "tools + process" path)

Keep v1's single `create_react_agent` loop. Add a small set of
...
- Smallest delta from v1 — one new tool class, no new graph nodes, no
...
- Compensation, the most error-prone flow in v0/v1, becomes a deterministic
...
- The agent is still responsible for *choosing* which validator to
  ...
  validator. v2 must enforce "validator-before-write" in the prompt and
...
- The hardest failure mode in v0/v1 was *applying* the rule, not
...
| Lines of code change from v1 | ~200 (new tools) | ~800 (router + 4 agents + handoff) | ~500 (graph rewrite) |
...
| Risk of regressing what v1 fixed | Low | Medium (different prompt = different tone failures) | Low |

---

## Recommendation: **Option A**

Adopt the **single agent + validator tools** structure for v2.

The reasoning:

1. **The failure class to fix is rule-application, not routing.** The
   v0/v1 evals show the agent usually identifies *which flow* correctly
...
2. **Decision-tree §8 explicitly anticipates this.** The
   ...
   v2 is the operationalization of that paragraph.

3. **Workshop pedagogy.** v1 added pure prompting (structure + principles +
   tone). v2 adds *tools and process* — a different axis. v3 (if we get
...
**When to revisit:** If v2's eval still shows persistent flow-mixing
errors (the model picks the wrong flow), Option B becomes warranted.
That's a v3 conversation, decided by the v2 eval signal.
...
Green = new in v2. Orange = existing tool whose schema is tightened in
v2 (`cabin: Literal[...]`, airport-code validators, explicit
`reason: Literal[...]` on `cancel_reservation`).

The graph is structurally still v1's ReAct loop. The change is in
...
| Policy section | Decision-tree branch | Structural home in v2 |
|---|---|---|
...
| Universal: verify user claims | §0 | Prompt rule (`<operating_principles>`) — v1 already has it |
...
| Tone (concise, plain outcomes) | n/a | `<response_style>` (v1) |
...
- **Final-message critic** (v1's `prompting-best-practices.md` flagged P5
  ...
  depends on v2's residual fabrication rate.
- **Specialist subagents (Option B)** as a future v3. If v2's eval
...
- [LangGraph — Multi-agent: Hierarchical Agent Teams](https://langchain-ai.github.io/langgraph/tutorials/multi_agent/hierarchical_agent_teams/) — the canonical
  ...
  for Option B; informs what we'd reach for in v3 if needed.
- [LangGraph — Multi-agent Network (handoffs)](https://langchain-ai.github.io/langgraph/tutorials/multi_agent/multi-agent-collaboration/) — the network/swarm
  ...
  for v2: airline support has bounded, predictable branching where
