...
- Relocate the current agent into a **versioned variant layout**: move `src/agent/` → `src/agents/v1/`, and introduce a small variant registry so future variants (`v2`, `v3`, …) can be added without disturbing `v1` and the eval CLI can pick which variant to run.
...
- Add an **LLM-as-judge evaluator** that scores a whole transcript per task. **v1 asserts only on `nl_assertions`** (50/50 tasks have these). Tool-call sequencing and DB end-state checks are intentionally deferred: tool calls are an agent implementation detail we don't want to assert on, and DB checking can land in a later change.
...
- `agent-variants`: versioned agent layout under `src/agents/v<N>/` with a uniform factory contract (`make_agent(store, llm) -> CompiledGraph`) and a registry that lets the runner instantiate any registered variant by id. `v1` is the existing code, relocated unchanged.
