...
- [x] 1.1 Add `src/agents/v2/pending_actions.py` with shared typed
...
- [x] 2.1 Add `src/agents/v2/subagents/schemas.py` with `ReadyToAct`,
...
- [x] 2.2 Implement `booking_specialist(input, store)` in
      `src/agents/v2/subagents/booking_specialist.py`. Encode the
...
- [x] 2.3 Implement `cancellation_specialist(input, store)` in
      `src/agents/v2/subagents/cancellation_specialist.py`. Encode the
...
- [x] 2.4 Implement `modification_specialist(input, store)` in
      `src/agents/v2/subagents/modification_specialist.py` covering the
...
- [x] 2.5 Implement `compensation_specialist(input, store)` in
      `src/agents/v2/subagents/compensation_specialist.py`. Return
...
- [x] 3.1 Add `src/agents/v2/tools.py` exposing exactly the 8 LLM-facing
...
- [x] 3.4 Add `src/agents/v2/graph.py` with `make_agent(store, llm)` that
      binds the 8 tools to `llm`, wires `create_react_agent`, and
      returns a `CompiledStateGraph`. Mirror the v1 entry-point shape
      so the eval CLI and Chainlit app pick it up identically.
- [x] 3.5 Register `"v2": _v2_make_agent` in
...
- [x] 4.2 Pass an instruction to the user simulator (sim prompt
      addendum, scoped to v2 runs) telling it that echoing the tag
...
- [x] 4.4 When the sim's reply does **not** echo the tag, leave the
      ...
      (pivot/reject path).
- [x] 4.5 Make the v2 sim-prompt addendum a no-op for v0/v1 agents so
...
- [x] 5.1 In the Chainlit app entrypoint (`app.py`), detect when the
      active agent is `v2` and, on each agent message, parse for the
...
- [x] 6.1 Add `tests/test_v2_agent.py` with a shared `make_v2_with_mock_llm`
...
- [x] 7.1 Update the top-level [CLAUDE.md](../../../CLAUDE.md) "Where
      to look" table with a row for `src/agents/v2/architecture.md`
      and `src/agents/v2/testing.md` (architecture row may already
      exist — verify).
...
- [x] 7.3 Run `uv run pytest tests/test_v2_agent.py -v` and confirm
      all seven cases pass.
- [x] 7.4 Run `uv run python -m src.eval.run --agent v2 --task-id 0`
