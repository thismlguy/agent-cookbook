...
v0 and v1 are single ReAct loops that carry the full airline policy in
...
The v2 design in `src/agents/v2/architecture.md` proposes a different
...
- Add a `v2` agent variant alongside `v0` and `v1`, registered in
  `src/agents/__init__.py` via `make_agent(store, llm)`.
- Add `src/agents/v2/subagents/` housing one Python file per
...
- Add `execute_pending_action(action_id, store)` as a plain Python
  ...
  `update_reservation_*`).
- Restrict the v2 orchestrator's tool surface to 8 tools: 3 info
...
- Wire up the eval runner's user simulator to echo
  ...
  sees it.
- Add `tests/test_v2_agent.py` end-to-end tests covering the seven
  scripted cases in `src/agents/v2/testing.md` (booking happy/deny,
...
### New Capabilities
- `v2-orchestrator-agent`: An orchestrator agent that owns the
...
- **Code (new):** `src/agents/v2/{graph.py,prompt.py,tools.py}`,
  `src/agents/v2/subagents/{booking,modification,cancellation,compensation,schemas}.py`,
  `src/agents/v2/pending_actions.py` (typed schemas +
  `execute_pending_action`), `tests/test_v2_agent.py`.
- **Code (modified):** `src/domain/store.py` (add
  `pending_actions: dict[str, PendingAction]` field);
  `src/agents/__init__.py` (register `v2`); `src/runner/runner.py`
...
- **Eval:** the existing eval suite gains a v2 row; ground-truth
  replay still works because the same underlying write tools fire
  with the same args — only the surface above them changes.
- **No breaking changes** to v0 or v1: they remain untouched and
  selectable via `--agent v0` / `--agent v1`.
