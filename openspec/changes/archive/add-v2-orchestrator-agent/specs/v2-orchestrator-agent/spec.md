...
The system SHALL provide a `v2` agent variant whose orchestrator is a
...
- **WHEN** the eval CLI is invoked with `--agent v2`
- **THEN** `src.agents.get_variant("v2")` returns the v2 factory and the
  CLI proceeds without an "unknown variant" error

#### Scenario: Orchestrator delegates a policy decision

- **WHEN** the user asks the v2 agent to cancel a reservation
...
be registered as LLM-callable tools for the v2 agent.

#### Scenario: LLM cannot directly invoke a write tool

- **WHEN** the v2 graph is compiled and its bound tool list is inspected
...
The system SHALL include `tests/test_v2_agent.py` with tests
exercising the seven scripted cases from
`src/agents/v2/testing.md`:
