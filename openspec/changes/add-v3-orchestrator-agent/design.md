...
The canonical v3 design lives in
[`src/agents/v3/architecture.md`](../../../src/agents/v3/architecture.md);
the test plan lives in
[`src/agents/v3/testing.md`](../../../src/agents/v3/testing.md). This
...
The earlier `propose-v3-architecture` change explored three shapes
(validator tools, supervisor + specialists, hybrid subgraphs) and
recommended validators (Option A). The architecture.md in v3/
...
- Must coexist with `v1` and `v2`. Both remain selectable; no shared
  prompt, no shared graph.
- Per-session `Store` is mutable and in-memory; v3 adds
...
- Implement v3 exactly as described in `src/agents/v3/architecture.md`:
...
- Cover the seven scripted end-to-end cases in `testing.md` with
  mocked-LLM tests in `tests/test_v3_agent.py`.
- Register `v3` in `src/agents/__init__.py` so it's selectable via
  `uv run python -m src.eval.run --agent v3`.

**Non-Goals:**

- Replacing v1 or v2. Both continue to ship.
- Re-running or updating any published-runs evaluation rows. v3's
...
- A final-message critic (the v2 prompting-best-practices document
  raised this; it stays out of scope for v3).
...
The seven cases in `testing.md` are end-to-end through the v3
...
(`uv run python -m src.eval.run --agent v3`), not in pytest. The
...
- **[Trade-off] Doubles the work for a new flow.** Adding a
  ...
  new test case. Compared to v1/v2 where adding a flow was just
...
- **[Trade-off] Frontend has to join the store at card-render
  ...
  tightly than v1/v2's "agent narrates everything". Acceptable
  because the existing Chainlit UI already imports from
  `src.domain` for other displays.

## Migration Plan

- v3 ships alongside v1 and v2. No code is removed.
- New `Store.pending_actions` field defaults to `{}` so existing
  v1/v2 sessions are unaffected.
- Eval CLI: `--agent v3` becomes available; the default agent
  remains `v1`.
- Chainlit UI: a feature-flag check (`agent_variant == "v3"`)
  switches the message-render path to look for
  `<confirmation_card>` tags. v1/v2 sessions go through the
  existing prose-only path.
- No data migration. `published-runs/` is unaffected.
- Rollback: revert this change. v1/v2 keep working; the
